        ...
        <tbody>
            @forelse($cities as $index => $city)
                <tr>
                    ...
                    <x-td>{{ $city->name }}</x-td>
                    <x-td>
                        <button wire:click="toggleActive('{{ $city->id }}')" class="text-xs {{ $city->isActive ? 'text-green-600' : 'text-red-600' }}">
                    ...
                    <x-td>
                        ...
                        <x-danger-button wire:click="forceDelete('{{ $city->id }}')" wire:loading.attr="disabled" class="ml-2">
                            Kalıcı Sil
                        </x-danger-button>
                    @else
                        <x-secondary-button wire:click="$dispatch('openEditModal', { id: '{{ $city->id }}' })" wire:loading.attr="disabled">
                            Düzenle
                        </x-danger-button>
                        <x-danger-button wire:click="$dispatch('openDeleteModal', { id: '{{ $city->id }}' })" wire:loading.attr="disabled" class="ml-2">
                            Sil
                        </x-danger-button>
                    @endif
                    </x-td>
