<div>
  ...
  <div class="grid grid-cols-6 gap-4 p-4 mb-5">
    ...
    <x-select wire:model.live="adStatusFilter" class="min-w-32">
        <option value="all">Tümü</option>
        <option value="with_ads">İlanlı</option>
        <option value="without_ads">İlansız</option>
    </x-select>
    ...
    <x-table>
        <thead>
            <tr>
                ...
                <x-th>Lokasyon</x-th>
                <x-th class="text-center">İlan Durumu</x-th>
        ...
        <tbody>
            @foreach ($portfolios as $portfolio)
                <tr class="relative group">
                    ...
                    <x-td>
                        <br> {{ $portfolio->state->name }} > {{ $portfolio->city->name }} >
                        {{ $portfolio->district->name }}
                        <br> ({{ $portfolio->lot }} / {{ $portfolio->parcel }})

                    </x-td>
                       <!-- Yeni ilan durumu sütunu -->
   <x-td class="text-center">
    <span class="{{ $portfolio->ads->isNotEmpty() ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold' }}">
        {{ $portfolio->ads->isNotEmpty() ? 'İlanlı' : 'İlansız' }}
    </span>
</x-td>
                    <x-td class="text-center">
                        @if ($priceEditing === $portfolio->id)
                            <!-- Fiyat düzenleme modundaysa input göster -->
                            <input type="number" wire:model.lazy="newPrice"
                        ...
                        @else
                            ...
                            <span class="font-bold text-black cursor-pointer text-md"
                                wire:click="editPrice({{ $portfolio->id }}, {{ $portfolio->price }})">
                            ...
                            @if ($portfolio->status === 'Kiralık' && $portfolio->additional_fees)
                                <span class="mt-1 text-sm text-black font-semibold">
                        ...
                        <!-- Kategoriye bağlı olarak alan bilgisi gösterme -->
                        <span class="font-extrabold text-red-500">
                            @if ($portfolio->category_id == $landCategoryId)
                                {{ number_format($portfolio->area_m2, 0) }} m²
                            @elseif ($portfolio->category_id == $businessCategoryId)
                                {{ number_format($portfolio->business->closed_area ?? 0, 0) }} m²
                            @endif
                        </span>
                    </x-td>
                    ...
                    @if ($categoryFilter == $businessCategoryId)
                        <x-td>
                            <div class="grid grid-cols-2 gap-x-4">
                                ...
                                <div class="font-semibold text-black">: {{ $portfolio->business->open_area ?? 'Boş' }}
                                    m²</div>

                                <div class="font-bold text-black">Kapalı Alan</div>
                                <div class="font-semibold text-black">:
                                    {{ $portfolio->business->closed_area ?? 'Boş' }} m²
                                </div>

                                <div class="font-bold text-black">İşletme Alanı</div>
                                <div class="font-semibold text-black">:
                                    {{ $portfolio->business->business_area ?? 'Boş' }} m²
                                </div>

                                <div class="font-bold text-black">Ofis Alanı</div>
                                <div class="font-semibold text-black">:
                                    {{ $portfolio->business->office_area ?? 'Boş' }} m²
                                </div>
                    ...
                    @if ($categoryFilter == $businessCategoryId)
                        <x-td>
                            <div class="grid grid-cols-2 gap-x-4">
                                ...
                                <div class="font-semibold text-black">:
                                    {{ $portfolio->business->floor_count ?? 'Boş' }}
                                </div>

                                <div class="font-bold text-black">Elektrik</div>
                                <div class="font-semibold text-black">:
                                    {{ $portfolio->business->electricity_power ?? 'Boş' }} KWA
                                </div>

                                <div class="font-bold text-black">Durumu</div>
                                <div class="font-semibold text-black">:
                                    {{ $portfolio->business->usage_status ?? 'Boş' }}
                                </div>
                                ...
                                <div class="font-semibold text-black">:
                                    {{ $portfolio->business->building_year ?? 'Boş' }}
                                </div>
                            </div>
                        </x-td>
                    @endif
